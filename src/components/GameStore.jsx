...
export default function GameStore({onSelect}) {
  return <main className="store-shell min-h-screen overflow-hidden px-4 py-8 sm:px-8">
    ...
    <div className="relative mx-auto max-w-6xl">
      ...
      <section className="grid gap-6 lg:grid-cols-2">{games.map(({id,eyebrow,title,subtitle,players,time,tags,icon:Icon,tone})=><article key={id} className={`game-box game-box-${tone}`}>
        ...
        <div className="relative z-10 flex h-full flex-col p-6 sm:p-8"><p className="text-xs font-black tracking-[.18em] text-[#8f6075]">{eyebrow}</p><h2 className="font-display mt-3 text-3xl font-black text-[#342a43]">{title}</h2><p className="mt-3 max-w-sm leading-7 text-[#655e6e]">{subtitle}</p><div className="mt-5 flex flex-wrap gap-2">{tags.map(t=><span key={t} className="rounded-full border border-white/70 bg-white/65 px-3 py-1.5 text-xs font-bold text-[#60566c]">{t}</span>)}</div><div className="mt-auto flex items-end justify-between gap-4 pt-10"><div className="flex gap-4 text-sm font-semibold text-[#655e6e]"><span className="flex items-center gap-1.5"><Users className="h-4 w-4"/>{players}</span><span className="flex items-center gap-1.5"><Clock3 className="h-4 w-4"/>{time}</span></div><button onClick={(event)=>onSelect(id,event)} className="game-select-button rounded-full bg-[#3d324c] px-6 py-3 text-sm font-black text-white shadow-lg transition hover:-translate-y-0.5 hover:bg-[#564461]">เลือกเกมนี้ →</button></div></div>
      </article>)}</section>
